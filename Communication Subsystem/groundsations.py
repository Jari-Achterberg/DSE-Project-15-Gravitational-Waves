...
#     dist_squared = (Rs - Re * np.cos(long) * np.cos(lat))**2 + (Re * np.sin(long) * np.cos(lat))**2 + (Re * np.sin(lat))**2
#     dist = np.sqrt(dist_squared)
#     return dist

def GStoSAT2(long_gs,lat_gs,long_sat):
    Re =  6371     # [km]
    Rs = 35786+Re  # [km]
    lat  = lat_gs*np.pi/180.
    long = abs(long_gs-long_sat)*np.pi/180.
    gamma = np.arccos( np.sin(0)*np.sin(lat) + np.cos(0)*np.cos(lat)*np.cos(long))
    dist = np.sqrt(Re*Re + Rs*Rs - 2*Rs*Re*np.cos(gamma))
    return dist
...
plt.xlim(-180,180)
plt.ylim(-satCoverage,satCoverage)
...
if geoSats(centerSat0)[1] > 180:
    plt.plot((-180+checkOverlap(geoSats(centerSat0)[1])),0,color='tomato',marker='o',markersize=13,label='Satellite B')
else:
...
if geoSats(centerSat0)[2] < -180:
    plt.plot((180-checkOverlap(geoSats(centerSat0)[2])),0,color='forestgreen',marker='o',markersize=13,label='Satellite C')
else:
...
dist01 = GStoSAT2(ddCoordsTab[1][0],ddCoordsTab[1][1],geoSats(centerSat0)[0])

# Distance from Sat B to Kourou [1]
dist11 = GStoSAT2(ddCoordsTab[1][0],ddCoordsTab[1][1],geoSats(centerSat0)[1])

# Distance from Sat B to Villafranca [2]
dist12 = GStoSAT2(ddCoordsTab[2][0],ddCoordsTab[2][1],geoSats(centerSat0)[1])

# Distance from Sat B to Redu [3]
dist13 = GStoSAT2(ddCoordsTab[3][0],ddCoordsTab[3][1],geoSats(centerSat0)[1])
...
dist21 = GStoSAT2(ddCoordsTab[0][0],ddCoordsTab[0][1],geoSats(centerSat0)[2])
